...
namespace std
{
  template <> struct default_delete<X509>       { void operator () (X509* n)      { X509_free(n);     }};
  template <> struct default_delete<X509_REQ>   { void operator () (X509_REQ* n)  { X509_REQ_free(n); }};
...
public:
  ...
  SSL_CTX* lookup_and_create(const char *servername, void *edata, bool &wontdo) {
    ...
    if (dataItr != cnDataMap.end()) {
      ...
      auto& target_data = cnDataMap[commonName];
  ...
  void setup_data_ctx(const std::string &commonName, std::queue<void*> &localQ, std::unique_ptr<SSL_CTX> ctx, std::unique_ptr<X509> cert, const bool &wontdo) {
    TSMutexLock(list_mutex);
  ...
  void prepend(SslData* data) {
    ...
    if (data != nullptr) {
      ...
      if (head != data) {
        // Remove data from the list if it is in
        bool present = false;
  ...
  // Remove list node
  void remove(SslData* data) {
    std::unique_ptr<SslData> local;
    TSMutexLock(list_mutex);
    if (data != nullptr) {
  ...
  // Set scheduled flag
  int set_schedule(const std::string &commonName, bool flag) {
    int ret = -1;
    TSMutexLock(list_mutex);
    auto iter = cnDataMap.find(commonName);
    if (iter != cnDataMap.end()) {
...
static int
sign_X509(X509 *cert, EVP_PKEY *pkey, const EVP_MD *md)
{
  int rv;
  EVP_MD_CTX mctx;
...
mkcrt(X509_REQ *req, int serial)
{
  ...
  sign_X509(cert.get(), ca_pkey_scoped.get(), EVP_sha256());
